...
- [ ] Create AudioInputManager class
  - [ ] Implement getUserMedia() audio capture
  - [ ] Create AnalyserNode with FFT configuration
  - [ ] Implement getFrequencyBins() method
  - [ ] Implement getWaveformData() method
  - [ ] Add audio device enumeration
  - [ ] Add gain/volume control
- [ ] Create audio input permissions handling
...
- [ ] Create VisualizationEngine class
  - [ ] Implement Ripple mode (frequency → color, amplitude → brightness)
    - [ ] Directional ripple with origin control
    - [ ] Color history buffer (PushQueue mechanism)
  - [ ] Implement Spectrum mode (FFT bars, frequency analyzer)
    - [ ] Bin grouping and averaging
    - [ ] Peak hold with fade
  - [ ] **Implement Waveform mode (NEW - oscilloscope-style)**
    - [ ] Scrolling waveform display
    - [ ] Triggered mode with zero-crossing detection
    - [ ] Stereo overlay (L=green, R=cyan)
    - [ ] Technical overlays (RMS, peak, frequency estimate)
  - [ ] ~~Fire mode~~ (LOW PRIORITY - deferred to future)
  - [ ] Add smoothing/interpolation for 60fps
- [ ] Create canvas rendering system
  - [ ] Setup requestAnimationFrame loop
  - [ ] Implement efficient canvas drawing
  - [ ] Add FPS counter for debugging
  - [ ] Add visualization mode selector UI (Ripple/Spectrum/Waveform)

### Phase 3: LED Matrix Output
- [ ] Create LEDMatrixManager component
  - [ ] Matrix configuration UI (width, height, IP address)
  - [ ] 2D grid → 1D array conversion
  - [ ] Integration with WLED websocket bridge
  - [ ] Support multiple matrix configurations
  - [ ] Virtual matrix preview (similar to VirtualLEDStrip)
- [ ] Extend WLED bridge if needed
  - [ ] Verify matrix addressing works correctly
  - [ ] Add matrix dimension metadata to messages
  - [ ] Test with physical WLED matrix hardware

### Phase 4: UI & Controls
- [ ] Create LiveAudioVisualizer main component
  - [ ] Audio input device selector dropdown
  - [ ] Gain/sensitivity slider (0-200%, default 100%)
  - [ ] Visualization mode dropdown (Ripple, Fire, Spectrum)
  - [ ] **Ripple Direction Controls** (visible when Ripple mode selected):
    - [ ] Origin indicator overlay on canvas (semi-transparent circle)
    - [ ] Click/touch canvas to set origin point
    - [ ] Double-click to reset to center
    - [ ] Preset buttons: "Center", "Bottom-Up", "Left-Right", "Top-Down", "Right-Left"
    - [ ] Origin coordinates display (X: 0.5, Y: 0.5)
  - [ ] LED matrix toggle and configuration
  - [ ] Canvas visualization area (full-width, responsive height)
  - [ ] Match existing component design patterns (gradient buttons, rounded-lg)
- [ ] Add to Welcome screen as experimental feature (BEFORE Multiplayer Jam)
  - [ ] Route: `/dj-visualizer`
  - [ ] Card title: "DJ Audio Visualizer" or "Live Audio Visualizer"
  - [ ] Icon: `Waveform` from Lucide React (music waveform visualization)
  - [ ] Badge: `Beta` with orange/amber colors (`bg-orange-600/30 text-orange-300`)
  - [ ] Placement: First card in Experiments grid (lines 153-154 in WelcomeScreen.tsx)
  - [ ] Button text: "Open Visualizer" or "Launch Visualizer"

### Phase 5: Integration & Testing
- [ ] Add route to App.tsx (see implementation section below)
  - [ ] Import LiveAudioVisualizer component
  - [ ] Add `handleDJVisualizer()` navigation function
  - [ ] Add `onDJVisualizer` prop to WelcomeScreen
  - [ ] Add `/dj-visualizer` route
- [ ] Update WelcomeScreen.tsx (see implementation section below)
  - [ ] Add `onDJVisualizer` to props interface
  - [ ] Import `Waveform` icon from Lucide React
  - [ ] Add DJ Visualizer card as FIRST item in Experiments grid
...
**Planning Phase:** Initial proof of concept planning
**Completion Notes:**
- Story created for experimental live audio visualization module
- Architecture inspired by Cymaspace AudioLux patterns (ripple, fire, spectrum)
- Follows existing component patterns (APC40, JamSession, LEDStripManager)
- Integrates with existing WLED websocket bridge infrastructure
- Designed for DJ/VJ performance use cases
- Proof of concept focus: basic functionality, then iterate
